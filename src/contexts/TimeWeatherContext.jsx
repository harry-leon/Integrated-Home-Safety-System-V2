    ...
    const fetchWeather = async () => {
      try {
        ...
        const res = await fetch(
          ...
          `&current=temperature_2m,apparent_temperature,weather_code,relative_humidity_2m,wind_speed_10m,precipitation_probability,uv_index`
        );
        ...
        setWeather({
          ...
          uvIndex: uvIndex != null ? Math.round(uvIndex) : '--',
        });
      } catch (error) {
        console.error('Lỗi khi tải thời tiết', error);
        setWeather({ temp: '--', feelsLike: '--', desc: 'Lỗi kết nối', icon: 'cloud_off', humidity: '--', windSpeed: '--', precipProb: '--', uvIndex: '--' });
      }
    };
