    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        ">
          ...
          <div className="relative z-10">
            ...
            <p className="text-outline/70 mt-2 max-w-md text-sm">{t('sys_desc')}</p>
          </div>
          ...
          <div className="flex gap-10 mt-10 relative z-10">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 text-on-surface">
                ...
                <span className="font-bold tabular-nums terminal-cursor">99.8%</span>
              </div>
            </div>
          </div>
        ...
        ">
          ...
          <div className="space-y-3 flex-1 text-on-surface">
            {[
              {
                icon: weather.icon, color: 'text-tertiary',
                label: t('weather'), value: `${weather.temp}°C`, sub: weather.desc,
              },
              {
                icon: 'light_mode', color: 'text-primary',
                label: t('brightness'), value: '420 lx', sub: t('indoor'),
              },
              {
                icon: 'co2', color: 'text-green-500',
                label: t('gas'), value: '0.02%', sub: t('safe'),
              },
            ].map((item, i) => (
              <div
                key={i}
      ...
      ">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h3 className="text-base font-bold tracking-[0.1em] uppercase text-on-surface flex items-center gap-2">
              ...
              Báo cáo tuần · 7 ngày qua
            </h3>
            <p className="text-xs text-outline/60 mt-1 font-['Inter'] italic">
              {isLoadingSnap ? 'Đang tổng hợp...' : (weeklySnap?.progressSummary || 'Chưa có dữ liệu.')}
            </p>
          </div>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-5">
          {[
            { label: 'Tổng truy cập',    val: weeklySnap?.totalAccessThisWeek,    rate: weeklySnap?.accessChangeRate,  icon: 'login' },
            { label: 'Cảnh báo',         val: weeklySnap?.alertsThisWeek,         rate: weeklySnap?.alertChangeRate,   icon: 'warning' },
            { label: 'Nguy cấp',         val: weeklySnap?.criticalAlertsThisWeek, isError: (weeklySnap?.criticalAlertsThisWeek > 0), icon: 'crisis_alert' },
          ].map((item, idx) => (
            <div
              key={idx}
              className="
          ...
          <div className="cyber-card bg-surface-container-low p-5 rounded-2xl border border-outline-variant/10">
            <p className="text-[9px] text-outline/60 uppercase font-black tracking-[0.2em] mb-3">Xu hướng 7 ngày</p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        ">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-primary/70 text-[9px] font-bold hover:text-primary cursor-pointer uppercase tracking-[0.2em] transition-colors">
              {t('view_all')} →
            </span>
          </div>
          ...
          <div className="space-y-3">
            {isLoadingAlerts ? (
              ...
              <div className="p-8 text-center border border-dashed border-outline-variant/20 rounded-2xl">
                ...
                <p className="text-outline/60 font-medium text-sm">Hệ thống an toàn. Không có cảnh báo.</p>
              </div>
            ) : (
                ...
                >
                  ...
                  `}>
                    <span className={`material-symbols-outlined text-[18px] ${alert.severity === 'CRITICAL' ? 'text-error' : 'text-tertiary'}`}>
                      {alert.severity === 'CRITICAL' ? 'crisis_alert' : 'notifications_active'}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center">
                      <h4 className="font-bold text-on-surface text-xs uppercase tracking-wider">{alert.alertType}</h4>
                    ...
                    <p className="text-xs text-outline/60 mt-0.5 truncate">{alert.message}</p>
                  </div>
                  {/* severity badge */}
                  <span className={`
        ...
        <div className="cyber-card bg-surface-container rounded-[2rem] overflow-hidden flex flex-col shadow-sm border border-outline-variant/10 relative">
          ...
          <div className="p-5 relative">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[9px] text-green-400 font-bold uppercase tracking-wider">● ARMED</span>
            </div>
